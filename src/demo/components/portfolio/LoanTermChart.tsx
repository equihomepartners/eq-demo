...
const LoanTermChart: React.FC<LoanTermChartProps> = ({
  data,
  title = 'Loan Term Distribution',
  height = 200
}) => {
  // Validate data
  if (!data || !data.terms || !data.counts || !data.percentages || !data.colors ||
      data.terms.length === 0 ||
      data.counts.length === 0 ||
      data.terms.length !== data.counts.length ||
      data.terms.length !== data.percentages.length ||
      data.terms.length !== data.colors.length) {
    return (
  ...
  const barWidth = 100 / data.terms.length;
          ...
          {data.terms.map((term, index) => {
            ...
            return (
              <div key={index} className="flex flex-col items-center" style={{ width: `${barWidth}%` }}>
                ...
                ></div>
                <div className="text-xs text-gray-600 mt-2">{term}</div>
              </div>
            );
          })}
        </div>
